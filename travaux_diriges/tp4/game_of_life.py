...
if(nbp==1) :
    ...
    parallelization = "double" # "simple", "simplerestricted", "double"
...
class Grille:
    ...
    """
    def __init__(self, dim, init_pattern=None, color_life=pg.Color("purple"), color_dead=pg.Color("white")):
        import random
        self.dimensions = dim
...
class App:
    ...
    # Changing the grid of the app without creating a new one : 
    def newgrid(self, grid):
        self.grid = grid
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1 :
        choice = sys.argv[1]
    resx = init_pattern = dico_patterns[choice][0][0]
    resy = init_pattern = dico_patterns[choice][0][1]
    if len(sys.argv) > 3 :
        resx = int(sys.argv[2])
        resy = int(sys.argv[3])
    ...
    if(parallelization=="none"):
        while loop:
            ...
            if k >= nb_it:
                loop = False
            k += 1
            time_computing += t2-t1
            time_drawing += t3-t2
            time_total += t3-t1
    
    elif(parallelization=="simple"):
        begin = time.time()
        if(rank==0): # Display
            while loop:
                t2 = time.time()
    ...
    elif(parallelization=="simplerestricted"):
        begin = time.time()
        if(rank==0): #Display
            while loop:
                t2 = time.time()
    ...
    print(f"Parallelization: {parallelization}")   
    print(f"Temps calcul moyen : {time_computing/k} secondes")  
    print(f"Temps affichage moyen : {time_drawing/k} secondes ({k} itérations réelles)")
    print(f"Temps total pour {nb_it} itérations : {time_total}")
